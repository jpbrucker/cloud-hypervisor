...
impl Vm {
    ...
    pub fn new_from_memory_manager(
        ...
        cfg_if::cfg_if! {
            if #[cfg(feature = "tdx")] {
                ...
                let arm_rme_enabled = config.lock().unwrap().is_arm_rme_enabled();
                let force_iommu = arm_rme_enabled;
            } else {
    ...
    pub fn create_hypervisor_vm(
        ...
        let vm = hypervisor
            .create_vm_with_caps_and_memory(
                confidential_vm,
                #[cfg(feature = "sev_snp")]
                mem_size,
            )
    ...
    pub fn boot(&mut self) -> Result<()> {
        ...
        if self.config.lock().unwrap().is_arm_rme_enabled() {
            ...
            for (addr, data) in self.memory_manager.lock().unwrap().boot_data() {
                ...
                self.vm
                    .arm_rme_realm_populate(addr.0, data.size as u64)
                    .map_err(Error::PopulateArmRme)?;
            }
