...
impl vm::Vm for KvmVm {
    ...
    //
    #[cfg(feature = "arm_rme")]
    fn arm_rme_realm_populate(&self, addr: u64, size: u64) -> vm::Result<()> {
    ...
    #[cfg(feature = "arm_rme")]
    fn arm_rme_init_ripas(&self, addr: u64, size: u64) -> vm::Result<()> {
        let aligned_addr = addr & !0xfff;
        let aligned_size = (size + 0xfff) & !0xfff;
...
impl hypervisor::Hypervisor for KvmHypervisor {
    ...
    /// let vm = hypervisor.create_vm_with_caps_and_memory(false).unwrap();
    /// ```
    ...
    fn create_vm(&self) -> hypervisor::Result<Arc<dyn vm::Vm>> {
        self.create_vm_with_caps_and_memory(
            false,
            #[cfg(feature = "sev_snp")]
            0,
        )
...
impl cpu::Vcpu for KvmVcpu {
    ...
    fn rec_finalize(&self) -> cpu::Result<()> {
        self.vcpu_finalize(KVM_ARM_VCPU_REC as i32)
    }
}
